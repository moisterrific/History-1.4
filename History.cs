...
namespace History
{
	...
	public class History : TerrariaPlugin
	{
		...
		void Queue(string account, int X, int Y, byte action, ushort data = 0, byte style = 0, short paint = 0, string text = null, int alternate = 0, int random = 0, bool direction = false)
		{
            ...
            //if (TShock.Server == ServerType.Freebuild)
            //    ServerUtils.Freebuild.FreebuildMain.TileActions.Add(new Action() { account = account, x = X, y = Y, action = action });
        }
		...
		void logEdit(byte etype, ITile tile, int X, int Y, ushort type, string account, List<Vector2> done, byte style = 0, int alt = 0, int random = -1, bool direction = false)
		{
			switch (etype)
			{
				case 21:
					{
						if (tile.active() || Main.tileCut[Main.tile[X, Y].type] && type != 127)
						{
							//use queue directly to avoid history accounting for furniture above the tiles normally getting broken
							Queue(account, X, Y, 0, type);
						...
						case PacketTypes.Tile:
							{
								...
								ushort type = (ushort)reader.ReadInt16();
								byte style = reader.ReadByte();
								//TShock.Players[e.Msg.whoAmI]?.SendInfoDebug("etype: {0} x: {1} y: {2} type: {3} style: {4}", etype, X, Y, type, style);
		...
		{
			TShockAPI.Commands.ChatCommands.Add(new Command("history.get", HistoryCmd, "history", "h"));
			TShockAPI.Commands.ChatCommands.Add(new Command("history.prune", Prune, "prunehist"));
			TShockAPI.Commands.ChatCommands.Add(new Command("history.reenact.self", Reenact, "reenact"));
			TShockAPI.Commands.ChatCommands.Add(new Command("history.rollback.self", Rollback, "rollback", "rb"));
			TShockAPI.Commands.ChatCommands.Add(new Command("history.rollback.self", Undo, "rundo"));
		...
		{
			if (e.Player.HasPermission("history.rollback.others"))
			{
			...
			else
			{
				if (e.Parameters.Count != 2)
				{
					e.Player.SendErrorMessage("Invalid syntax. Proper syntax: /reenact <time> [radius].");
					return;
				}
				var parameters = e.Parameters.ToList();
				e.Parameters.Clear();
				e.Parameters.Add(e.Player.Account.Name);
				e.Parameters.AddRange(parameters);
			}
			int radius = 10000;
			int time;
			if (!TShock.Utils.TryParseTime(e.Parameters[1], out time) || time <= 0)
			...
			{
				CommandQueue.Add(new RollbackCommand(e.Parameters[0], time, radius, e.Player, null, true));
			}
		}
		void Rollback(CommandArgs e)
		{
			var rollbackOthers = e.Player.HasPermission("history.rollback.others");
			if (rollbackOthers)
			{
				if (e.Parameters.Count < 2)
				{
			...
			else
			{
				if (e.Parameters.Count != 2)
				{
					e.Player.SendErrorMessage("Invalid syntax. Proper syntax: /rollback <time> [radius].");
					return;
				}
				var parameters = e.Parameters.ToList();
				e.Parameters.Clear();
			...
			if (!TShock.Utils.TryParseTime(e.Parameters[1], out time) || time <= 0)
				...
				CommandQueue.Add(new RollbackCommand(e.Parameters[0], time, radius, e.Player, e.Parameters[1], false));
		}
